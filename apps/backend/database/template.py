...
def get_template_by_id(template_id: int) -> dict | None:
    with get_db() as conn:
        row = (
            conn.execute(
                text("SELECT * FROM doc_templates WHERE id=:id"), {"id": template_id}
            )
            .mappings()
            .one_or_none()
        )
...
def get_template_by_code(code: str) -> dict | None:
    with get_db() as conn:
        row = (
            conn.execute(
                text("SELECT * FROM doc_templates WHERE code=:code"), {"code": code}
            )
            .mappings()
            .one_or_none()
        )
...
def get_all_templates(with_fields=False) -> list:
    with get_db() as conn:
        rows = (
            conn.execute(text("SELECT * FROM doc_templates ORDER BY sort_order, id"))
            .mappings()
            .all()
        )
    result = []
...
def update_template(template_id: int, **kwargs) -> bool:
    ...
    if not updates:
        return False
    if "classification_keywords" in updates and isinstance(
        updates["classification_keywords"], list
    ):
        updates["classification_keywords"] = json.dumps(
            updates["classification_keywords"], ensure_ascii=False
        )
...
def delete_template(template_id: int) -> bool:
    with get_db() as conn:
        row = conn.execute(
            text("SELECT is_system FROM doc_templates WHERE id=:id"),
            {"id": template_id},
        ).fetchone()
        if not row or row[0]:
            return False
        ...
        conn.execute(
            text("DELETE FROM template_examples WHERE template_id=:id"),
            {"id": template_id},
        )
        conn.execute(
            text("DELETE FROM doc_templates WHERE id=:id"), {"id": template_id}
        )
    return True
...
def get_example_image(template_id: int) -> str | None:
    with get_db() as conn:
        row = conn.execute(
            text("SELECT image_path FROM template_examples WHERE template_id=:id"),
            {"id": template_id},
        ).fetchone()
...
def get_ocr_patterns(template_code: str) -> list[dict]:
    ...
    with get_db() as conn:
        row = conn.execute(
            text("SELECT id FROM doc_templates WHERE code=:code"),
            {"code": template_code},
        ).fetchone()
        if not row:
            return []
    ...
    return [
        {"field_name": r[0], "ocr_pattern": r[1], "field_category": r[2]} for r in rows
    ]
