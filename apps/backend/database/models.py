...
class User(Base):
    ...
    role: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default="operator"
    )
...
class TemplateField(Base):
    ...
    field_category: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default="required"
    )
...
class TemplateExample(Base):
    ...
    template_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("doc_templates.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,
    )
...
class LeaveApplication(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, server_default="SUBMITTED"
    )
