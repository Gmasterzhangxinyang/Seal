...
def _generate_application_id() -> str:
    ...
    with get_db() as conn:
        ...
        ), {'prefix': f'LEAVE-{today}-%'}).mappings().one_or_none()
...
def create_leave_application(
    ...
    with get_db() as conn:
        from database.models import LeaveApplication
...
def get_leave_application(
    ...
    with get_db() as conn:
        ...
        ), {'application_id': application_id}).mappings().one_or_none()
    if not row:
...
def download_leave_application(
    ...
    fields_para = [
        [Paragraph("<b>申请编号</b>", small_style), Paragraph(app['application_id'], body_style)],
        [Paragraph("<b>学生姓名</b>", small_style), Paragraph(app['student_name'], body_style)],
        [Paragraph("<b>学　　号</b>", small_style), Paragraph(app['student_id'], body_style)],
        [Paragraph("<b>院　　系</b>", small_style), Paragraph(app['dept'] or '-', body_style)],
        [Paragraph("<b>请假类型</b>", small_style), Paragraph(app['leave_type'], body_style)],
        [Paragraph("<b>开始日期</b>", small_style), Paragraph(app['start_date'], body_style)],
        [Paragraph("<b>结束日期</b>", small_style), Paragraph(app['end_date'], body_style)],
        [Paragraph("<b>请假原因</b>", small_style), Paragraph(app['reason'], body_style)],
    ]
    ...
    qr_para = Paragraph("<b>二维码</b><br/><font size=8>（审批通过后有效）</font>", small_style)
