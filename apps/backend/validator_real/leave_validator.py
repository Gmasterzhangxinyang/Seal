...
def verify_leave_application(
    ...
    if status != "APPROVED":
        ...
        if status == "SUBMITTED":
            ...
            result.add_check(
                ...
                "warn",
                _SCORE_GENERAL_FAIL,
                "申请尚未完成 AI 自动审批，需人工审批",
            )
            result.warnings.append("申请尚未完成 AI 自动审批，需人工审批")
            # 不在这里 return，继续后续核验；最终决策会根据 risk_score >= 40 进入 REVIEW
        else:
            reason_map = {
                "REJECTED": "申请已被拒绝",
                "STAMPED": "申请已完成盖章",
    ...
    if not ocr_reason:
        result.add_check(
            "reason_check", "warn", 0, "请假原因字段缺失（不影响自动决策）"
        )
    ...
    if ocr_confidence >= _CONFIDENCE_HIGH:
        ...
        # OCR 置信度中等 — 记录但不影响决策，由 stamp.py 端重新识别
        result.add_check(
            "ocr_confidence_check",
            "warn",
            0,
            f"OCR 置信度中等 ({ocr_confidence:.2f}), 将重新识别",
        )
        result.warnings.append(f"OCR 置信度中等 ({ocr_confidence:.2f})，将重新识别")
    else:
        # OCR 置信度过低 — 记录并触发重新识别，不直接拒绝
        result.add_check(
            "ocr_confidence_check",
            "warn",
            0,
            f"OCR 置信度过低 ({ocr_confidence:.2f} < {_CONFIDENCE_MEDIUM})，将重新识别",
        )
        result.warnings.append(f"OCR 置信度过低 ({ocr_confidence:.2f})，将重新识别")
