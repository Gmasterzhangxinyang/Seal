...
def verify_leave_application(
    ...
    if not payload:
        result.add_check('qr_signature_check', 'fail', _SCORE_HARD_FAIL, '二维码内容解析失败')
        result.errors.append('二维码内容无效')
        return _finalize(result)

    if not verify_qr_payload(payload):
        result.add_check('qr_signature_check', 'fail', _SCORE_HARD_FAIL, '二维码签名验证失败，文档可能被篡改')
    ...
    with get_db() as conn:
        ...
        row = conn.execute(text(
            'SELECT * FROM leave_applications WHERE application_id=:id'
        ), {'id': application_id}).mappings().one_or_none()

    if not row:
        result.add_check('application_exists_check', 'fail', _SCORE_HARD_FAIL, f'申请记录不存在: {application_id}')
    ...
    result.add_check('application_status_check', 'pass', 0, f'申请已审批通过 (APPROVED)')

    # ── 检查4：重复盖章检测 ────────────────────────────────────────────────
    if app.get('stamped_at'):
        result.add_check('duplicate_stamp_check', 'fail', _SCORE_HARD_FAIL, '该申请已盖章，禁止重复盖章')
    ...
    if ocr_student_id and ocr_student_id != qr_student_id:
        result.add_check('student_id_match_check', 'fail', _SCORE_HARD_FAIL, 'OCR 学号与申请记录不一致')
    ...
    if ocr_name:
        if not _name_similar(app_name, ocr_name):
            ...
            result.add_check('student_name_match_check', 'fail', _SCORE_GENERAL_FAIL, '姓名与申请记录不一致')
            result.errors.append('姓名不一致')
        else:
            result.add_check('student_name_match_check', 'pass', 0, '姓名与申请记录一致')
    ...
    if ocr_type and app_type:
        if ocr_type != app_type:
            result.risk_score += _SCORE_GENERAL_FAIL
            result.add_check('leave_type_match_check', 'fail', _SCORE_GENERAL_FAIL, '请假类型与申请记录不一致')
            result.errors.append('请假类型不一致')
        else:
            result.add_check('leave_type_match_check', 'pass', 0, '请假类型与申请记录一致')
    ...
    if ocr_start and app_start and ocr_start != app_start:
        result.risk_score += _SCORE_GENERAL_FAIL
        result.add_check('start_date_match_check', 'fail', _SCORE_GENERAL_FAIL, f'开始日期不一致: OCR={ocr_start}, 申请={app_start}')
        result.errors.append('开始日期不一致')
        date_ok = False

    if ocr_end and app_end and ocr_end != app_end:
        result.risk_score += _SCORE_GENERAL_FAIL
        result.add_check('end_date_match_check', 'fail', _SCORE_GENERAL_FAIL, f'结束日期不一致: OCR={ocr_end}, 申请={app_end}')
    ...
    if not ocr_reason:
        ...
        result.add_check('reason_check', 'warn', _SCORE_WARNING_MEDIUM, '请假原因字段缺失')
    ...
    if ocr_confidence >= _CONFIDENCE_HIGH:
        result.add_check('ocr_confidence_check', 'pass', 0, f'OCR 置信度满足自动通过阈值 ({ocr_confidence:.2f} >= {_CONFIDENCE_HIGH})')
    elif ocr_confidence >= _CONFIDENCE_MEDIUM:
        result.risk_score += _SCORE_WARNING_MEDIUM
        result.add_check('ocr_confidence_check', 'warn', _SCORE_WARNING_MEDIUM, f'OCR 置信度中等 ({ocr_confidence:.2f}), 进入人工复审')
        result.warnings.append(f'OCR 置信度中等 ({ocr_confidence:.2f})')
    else:
        result.risk_score += _SCORE_GENERAL_FAIL
        result.add_check('ocr_confidence_check', 'fail', _SCORE_GENERAL_FAIL, f'OCR 置信度过低 ({ocr_confidence:.2f} < {_CONFIDENCE_MEDIUM})')
...
def _finalize(result: LeaveVerificationResult, qr_student_id: str = '') -> LeaveVerificationResult:
    """在早期失败时快速返回 REJECT"""
    if result.errors:
        result.decision = 'REJECT'
