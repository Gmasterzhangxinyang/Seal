...
def stats_data(session: dict = Depends(get_session)):
    with get_db() as conn:
        ...
        approved = conn.execute(text(
            "SELECT COUNT(*) FROM audit_log WHERE result='APPROVED'"
        )).scalar()
        rejected = conn.execute(text(
            "SELECT COUNT(*) FROM audit_log WHERE result='REJECTED'"
        )).scalar()
        pending_review = conn.execute(text(
            "SELECT COUNT(*) FROM audit_log WHERE result='PENDING_REVIEW'"
        )).scalar()
        pending_queue = conn.execute(text(
